describe("low level http 1 connection operations", function()
	...
	it(":read_request_line should fail on invalid request", function()
		local function test(chunk)
			...
			assert(s:flush())
			assert.same({nil, "invalid request line", ce.ENOMSG}, {c:read_request_line()})
		end
	...
	it(":read_request_line should allow a leading CRLF", function()
		local function test(chunk)
			...
			assert._not.same({nil, "invalid request line"}, {c:read_request_line()})
		end
		test("\r\nGET / HTTP/1.1\r\n")
	end)
	...
	it(":read_status_line should throw on invalid status line", function()
		local function test(chunk)
			...
			assert(s:write(chunk, "\r\n"))
			assert(s:flush())
			assert.same({nil, "invalid status line", ce.ENOMSG}, {c:read_status_line()})
		end
	...
	it(":read_headers_done should handle failure conditions", function()
		...
		do -- wrong byte
			...
			c:close()
			assert.same({nil, "invalid header: expected CRLF", ce.ENOMSG}, {s:read_headers_done()})
		end
		do -- wrong bytes
			local s, c = new_pair(1.1)
			c = c:take_socket()
