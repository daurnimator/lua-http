describe("http.request module", function()
	...
	describe(":use_proxy method", function()
		it("works with http proxies", function()
			local req = request.new_from_uri("http://example.com/")
			req:use_proxy("http://myproxy.com")
			assert.same("myproxy.com", req.host)
			assert.same(false, req.tls)
			assert.same("GET", req.headers:get ":method")
			assert.same("http", req.headers:get ":scheme")
			assert.same("example.com", req.headers:get ":authority")
			assert.same("http://example.com/", req.headers:get ":path")
		end)
		it("works with http proxies on OPTIONS requests", function()
			...
			assert.same("myproxy.com", req.host)
			assert.same(false, req.tls)
			assert.same("OPTIONS", req.headers:get ":method")
			assert.same("http", req.headers:get ":scheme")
			assert.same("example.com", req.headers:get ":authority")
			assert.same("http://example.com", req.headers:get ":path")
		end)
		it("adds proxy-authorization header", function()
			local basexx = require "basexx"
			local req = request.new_from_uri("http://example.com/")
			req:use_proxy("http://user:pass@myproxy.com")
			assert.same("basic " ..basexx.to_base64("user:pass"), req.headers:get "proxy-authorization")
		end)
		it("removes proxies when passed `nil`", function()
			local req1 = request.new_from_uri("http://example.com/?foo")
			local req2 = req1:clone()
			req2:use_proxy("http://myproxy.com")
			req2:use_proxy(nil)
			assert.same(req1.headers, req2.headers)
			assert.same(req1, req2)
		end)
		it("removes proxies when passed `nil` on OPTIONS requests", function()
			local req1 = request.new_from_uri("http://example.com")
			req1.headers:upsert(":method", "OPTIONS")
			req1.headers:upsert(":path", "*")
			local req2 = req1:clone()
			req2:use_proxy("http://myproxy.com")
			req2:use_proxy(nil)
			assert.same(req1.headers, req2.headers)
	...
	describe(":handle_redirect method", function()
		...
		it("doesn't drop proxy use within a domain", function()
			local orig_req = request.new_from_uri("http://example.com")
			orig_req:use_proxy("http://myproxy.com")
			local orig_headers = headers.new()
			orig_headers:append(":status", "302")
			orig_headers:append("location", "/foo")
			local new_req = orig_req:handle_redirect(orig_headers)
