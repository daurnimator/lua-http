describe("cookie module", function()
	...
	it(":get works", function()
		...
		assert(s:store("mysite.com", "/", true, true, key, value, params))
	...
	it("implements examples from spec", function()
		...
		assert(s:store("example.com", "/", true, true, http_cookie.parse_setcookie("SID=31d4d96e407aad42")))
		...
		assert(s:store("example.com", "/", true, true, http_cookie.parse_setcookie("SID=31d4d96e407aad42; Path=/; Domain=example.com")))
		assert.same("SID=31d4d96e407aad42", s:lookup("example.com", "/", true, true))
		assert.same("SID=31d4d96e407aad42", s:lookup("example.com", "/other", true, true))
		assert.same("SID=31d4d96e407aad42", s:lookup("subdomain.example.com", "/", true, true))
		assert.same("", s:lookup("other.com", "/", true, true))

		assert(s:store("example.com", "/", true, true, http_cookie.parse_setcookie("SID=31d4d96e407aad42; Path=/; Secure; HttpOnly")))
		assert(s:store("example.com", "/", true, true, http_cookie.parse_setcookie("lang=en-US; Path=/; Domain=example.com")))
		...
		assert(s:store("example.com", "/", true, true, http_cookie.parse_setcookie("lang=en-US; Expires=Wed, 09 Jun 2021 10:18:14 GMT")))
	...
	describe(":store uses correct domain", function()
		it("ignores leading '.' in domain", function()
			local s = http_cookie.new_store()
			assert.truthy(s:store("subdomain.example.com", "/", true, true, http_cookie.parse_setcookie("foo=bar; Domain=.example.com")))
		...
		;(http_cookie.store_methods.psl and it or pending)("checks against public suffix list", function()
			...
			local s = http_cookie.new_store()
			assert.falsy(s:store("foo.com", "/", true, true, http_cookie.parse_setcookie("foo=bar; Domain=com")))
		end)
		;(http_cookie.store_methods.psl and it or pending)("allows explicit domains even when on the public suffix list", function()
			...
			local s = http_cookie.new_store()
			assert.truthy(s:store("hashbang.sh", "/", true, true, http_cookie.parse_setcookie("foo=bar; Domain=hashbang.sh")))
		...
		it("doesn't domain-match a completely different domain", function()
			local s = http_cookie.new_store()
			assert.falsy(s:store("example.com", "/", true, true, http_cookie.parse_setcookie("foo=bar; Domain=other.example.com")))
		end)
		it("doesn't domain-match a subdomain when request is at super-domain", function()
			local s = http_cookie.new_store()
			assert.falsy(s:store("example.com", "/", true, true, http_cookie.parse_setcookie("foo=bar; Domain=subdomain.example.com")))
		end)
	end)
	describe("domain-match on lookup", function()
		it("matches domains correctly when host_only flag is true", function()
			local s = http_cookie.new_store()
			assert.truthy(s:store("s.example.com", "/", true, true, http_cookie.parse_setcookie("foo=bar")))
		...
		it("matches domains correctly when host_only flag is false", function()
			local s = http_cookie.new_store()
			assert.truthy(s:store("s.example.com", "/", true, true, http_cookie.parse_setcookie("foo=bar; Domain=s.example.com")))
			assert.same("bar", s:get("s.example.com", "/", "foo"))
	...
	describe(":store uses correct path", function()
		it("handles absolute set-cookie header", function()
			local s = http_cookie.new_store()
			assert.truthy(s:store("example.com", "/absolute/path", true, true, http_cookie.parse_setcookie("foo=bar; Path=/different/absolute/path")))
			assert.same("bar", s:get("example.com", "/different/absolute/path", "foo"))
		end)
		it("handles relative set-cookie path", function()
			local s = http_cookie.new_store()
			assert.truthy(s:store("example.com", "/absolute/path", true, true, http_cookie.parse_setcookie("foo=bar; Path=relative/path")))
		...
		it("handles relative set-cookie path with no request path", function()
			local s = http_cookie.new_store()
			assert.truthy(s:store("example.com", "?", true, true, http_cookie.parse_setcookie("foo=bar; Path=relative/path")))
			-- should default to /
			assert.same("bar", s:get("example.com", "/", "foo"))
		end)
		it("handles absolute set-cookie path with relative request path", function()
			local s = http_cookie.new_store()
			assert.truthy(s:store("example.com", "relative/path", true, true, http_cookie.parse_setcookie("foo=bar; Path=/absolute/path")))
		...
		it("handles relative request path and relative set-cookie header", function()
			local s = http_cookie.new_store()
			assert.truthy(s:store("example.com", "relative/path", true, true, http_cookie.parse_setcookie("foo=bar; Path=different/relative/path")))
			assert.same("bar", s:get("example.com", "/", "foo"))
		end)
	end)
	it("matches paths correctly", function()
		local s = http_cookie.new_store()
		assert.truthy(s:store("example.com", "/", true, true, http_cookie.parse_setcookie("foo=bar; Path=/path/subpath")))
	...
	it("prefers max-age over expires", function()
		local s = http_cookie.new_store()
		assert(s:store("example.com", "/", true, true, http_cookie.parse_setcookie("foo=bar; max-age=50; Expires=Thu, 01 Jan 1970 00:00:00 GMT")))
		assert.truthy(s:get("example.com", "/", "foo"))
		assert(s:store("example.com", "/", true, true, http_cookie.parse_setcookie("foo=bar; max-age=0; Expires=Tue, 19 Jan 2038 03:14:07 GMT")))
		assert.falsy(s:get("example.com", "/", "foo"))
	end)
	it("supports HttpOnly attribute", function()
		local s = http_cookie.new_store()
		assert.falsy(s:store("example.com", "/", false, true, http_cookie.parse_setcookie("foo=bar; HttpOnly")))
		assert.truthy(s:store("example.com", "/", true, true, http_cookie.parse_setcookie("foo=bar; HttpOnly")))
	...
	it("supports Secure attribute", function()
		local s = http_cookie.new_store()
		assert.falsy(s:store("example.com", "/", true, false, http_cookie.parse_setcookie("foo=bar; Secure")))
		assert.truthy(s:store("example.com", "/", true, true, http_cookie.parse_setcookie("foo=bar; Secure")))
	...
	describe("cookie fixing mitigation", function()
		it("ignores already existing path", function()
			local s = http_cookie.new_store()
			assert.truthy(s:store("example.com", "/path/subpath/foo", true, true, http_cookie.parse_setcookie("foo=bar; Path=/path; Secure")))
			assert.falsy(s:store("example.com", "/path/subpath/foo", true, false, http_cookie.parse_setcookie("foo=bar; Path=/path")))
		end)
	end)
end)
